...
climate_to_tokens = {}
with open("../climatedict.csv", encoding="utf-8",newline="") as csvfile:
...
caption_files = []
for i in trange(12):
    caption_file = f"F:cah400M\cah400M{i+1}.txt"
    with open(caption_file,encoding="utf-8") as capfile:
        length = len(list(capfile))
    caption_files.append((i,caption_file,length))
        ...
        for _ in trange(length):
            ...
            for key in climate_to_tokens.keys():
                ...
                if climate_tokens.issubset(caption_tokens_set):
                    label_count[key] += 1
    with open(f"../label_counts/climate_tokens_labelcount_simp{idx}.json", 'w',encoding="utf-8") as handle:
...
def main():
    ...
    # r = process_map(f,caption_files, max_workers=12)
